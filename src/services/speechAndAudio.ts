...
export class SpeechAndAudioService {
  ...
  public async startRecording(
    ...
    try {
        ...
        try {
          ...
          this.speechRecognition.onresult = (event: any) => {
            ...
            for (let i = event.resultIndex; i < event.results.length; ++i) {
              ...
              if (item.isFinal) {
                accumulatedTranscript += (accumulatedTranscript ? ' ' : '') + item[0].transcript;
              } else {
                interim += item[0].transcript;
              }
            }
            const currentCombined = (accumulatedTranscript + (interim ? ' ' + interim : '')).trim();
            if (currentCombined) {
              onTranscript(currentCombined);
            }
          };
      ...
      if (typeof navigator !== 'undefined' && navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        if (AudioContextClass) {
          ...
          const analyzeLoop = () => {
            ...
            const normalized = Math.min(1.0, Math.max(0.15, avg / 120));
      ...
      console.warn('Microphone access notice. Using fallback recorder.', err);
      this.startFallbackLoop(onTimeUpdate, onWaveformUpdate);
      return true;
    }
  }
